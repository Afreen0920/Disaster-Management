...
export default function ChangePassword() {
  ...
  const submit = async (e) => {
    ...
    if (!strong.test(form.newPassword)) {
      return setMsg("New password must be 8+ chars with uppercase, lowercase, number & special char.");
    }
    if (form.newPassword !== form.confirm) {
      return setMsg("Passwords do not match.");
    }
    const ok = await changePassword({ oldPassword: form.oldPassword, newPassword: form.newPassword });
